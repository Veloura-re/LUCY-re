...
export function ParentBillingView({ user, students }: ParentBillingViewProps) {
    ...
    const handlePay = async (invoiceId: string) => {
        ...
        try {
            const res = await fetch('/api/school/finance/pay', {
                method: 'POST',
                                                ...
                                                <div className="pt-6 border-t border-zinc-900/50">
                                                    ...
                                                    {invoice.status !== 'PAID' ? (
                                                        <Button
                                                            ...
                                                            className="w-full h-14 bg-white text-black hover:bg-zinc-200 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all"
                                                        >
                                                            {processingId === invoice.id ? "Processing..." : "Settle Outstanding"}
                                                        </Button>
                                                    ) : (
